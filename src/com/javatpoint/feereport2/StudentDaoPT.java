...
public class StudentDaoPT {

	public static int save(StudentPT s){
		int status=0;
		try{
			Connection con= AccountantDaoPT.getCon();
	...
	public static int update(StudentPT s){
		int status=0;
		try{
			Connection con= AccountantDaoPT.getCon();
	...
	public static List<StudentPT> view(){
		List<StudentPT> list=new ArrayList<StudentPT>();
		try{
			Connection con= AccountantDaoPT.getCon();
			PreparedStatement ps=con.prepareStatement("select * from feereport_student");
			ResultSet rs=ps.executeQuery();
			while(rs.next()){
				StudentPT s=new StudentPT();
	...
	public static StudentPT getStudentByRollno(int rollno){
		StudentPT s=new StudentPT();
		try{
			Connection con= AccountantDaoPT.getCon();
	...
	public static List<StudentPT> due(){
		List<StudentPT> list=new ArrayList<StudentPT>();
		try{
			Connection con= AccountantDaoPT.getCon();
